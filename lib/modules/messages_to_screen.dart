...
class MessagesToScreen extends StatefulWidget {
  MessagesToScreen({Key? key}) : super(key: key);

  @override
  State<MessagesToScreen> createState() => _MessagesToScreenState();
}

class _MessagesToScreenState extends State<MessagesToScreen> {
  var formKey = GlobalKey<FormState>();

  int selectedIndex = -1;
  Stream collectionStream =FirebaseFirestore.instance.collection('messagesTo').orderBy('timestamp', descending: true).limit(7).snapshots();
 ...
 Widget builtIteam(int index, MessagesModel data){
   ...
   Stream subcollectionStream =FirebaseFirestore.instance.collection('messagesTo/${data.name}/${data.name}').orderBy('timestamp', descending: true).snapshots();
